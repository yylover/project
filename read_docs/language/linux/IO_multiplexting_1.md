...
1. 每个使用ET工作模式的文件描述符都应该是非阻塞的，如果阻塞的话，那么读写操作会因为没有后续事件而一直等待。
2. ET相对于LT模式效率更高。

### EPOLLONESHOT
1. 因为即使使用ET工作模式，同一个socket的事件也可能出发多次，一次读取之后，某一线程正在处理，那么又有了新的数据，这时，会有一个新的线程处理，这不是我们期望的。我们期望一个SOCKET任一时刻都只被一个线程处理。
2. 注册了该事件后，该文件描述符上的可读可写异常事件只会触发一次，除非处理完后重置。
...
非阻塞连接connect,如果发生EINPROGRESS,连接又没有立刻建立，可以调用select,poll等监听socket的可读事件。select 返回好吧，再使用getsockopt清除文件描述符的错误。
